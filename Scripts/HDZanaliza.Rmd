...
wcl %>%
  with(wordcloud(word, n, max.words = 150, colors =  c("#56B4E9", "#E69F00"))) 
...
```{r oblak2, echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE, fig.height=5}
bigrams_united %>%
  group_by(bigram) %>%
  tally %>%
  filter(n < 58) %>%
  with(wordcloud(bigram, n, max.words = 150, colors =  c("#56B4E9", "#E69F00")))
```
